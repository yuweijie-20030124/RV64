...
#define SHAMT_MASK MUXDEF(CONFIG_ISA64, 0x3f, 0x1f)
...
#ifdef CONFIG_ISA64
static inline word_t divw(word_t src1, word_t src2) {
  int32_t dividend = (int32_t)src1;
  int32_t divisor = (int32_t)src2;
  if (divisor == 0) return SEXT((uint32_t)-1, 32);
  if (dividend == INT32_MIN && divisor == -1) return SEXT((uint32_t)INT32_MIN, 32);
  return SEXT((uint32_t)(dividend / divisor), 32);
}

static inline word_t divuw(word_t src1, word_t src2) {
  uint32_t dividend = (uint32_t)src1;
  uint32_t divisor = (uint32_t)src2;
  if (divisor == 0) return SEXT((uint32_t)-1, 32);
  return SEXT(dividend / divisor, 32);
}

static inline word_t remw(word_t src1, word_t src2) {
  int32_t dividend = (int32_t)src1;
  int32_t divisor = (int32_t)src2;
  if (divisor == 0) return SEXT((uint32_t)dividend, 32);
  if (dividend == INT32_MIN && divisor == -1) return 0;
  return SEXT((uint32_t)(dividend % divisor), 32);
}

static inline word_t remuw(word_t src1, word_t src2) {
  uint32_t dividend = (uint32_t)src1;
  uint32_t divisor = (uint32_t)src2;
  if (divisor == 0) return SEXT(dividend, 32);
  return SEXT(dividend % divisor, 32);
}
#endif
/*
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("000000? ????? ????? 101 ????? 00100 11", srli   , I, R(rd) = src1 >> (imm & SHAMT_MASK)); 
  INSTPAT("000000? ????? ????? 001 ????? 00100 11", slli   , I, R(rd) = src1 << (imm & SHAMT_MASK));
  INSTPAT("010000? ????? ????? 101 ????? 00100 11", srai   , I, R(rd) = (sword_t)src1 >> (imm & SHAMT_MASK)); 
  ...
  IFDEF(CONFIG_ISA64, INSTPAT("??????? ????? ????? 110 ????? 00000 11", lwu    , I, R(rd) = Mr(src1 + imm, 4)));
  IFDEF(CONFIG_ISA64, INSTPAT("??????? ????? ????? 011 ????? 00000 11", ld     , I, R(rd) = Mr(src1 + imm, 8)));
  INSTPAT("??????? ????? ????? 111 ????? 00100 11", andi   , I, R(rd) = src1 & imm); 
  INSTPAT("??????? ????? ????? 100 ????? 00100 11", xori   , I, R(rd) = src1 ^ imm); 
  INSTPAT("??????? ????? ????? 110 ????? 00100 11", ori    , I, R(rd) = src1 | imm);

  INSTPAT("0000000 ????? ????? 001 ????? 00110 11", slliw  , I, R(rd) = SEXT((uint32_t)src1 << BITS(imm, 4, 0), 32));
  INSTPAT("0000000 ????? ????? 101 ????? 00110 11", srliw  , I, R(rd) = SEXT((uint32_t)src1 >> BITS(imm, 4, 0), 32));
  INSTPAT("0100000 ????? ????? 101 ????? 00110 11", sraiw  , I, R(rd) = SEXT((int32_t)src1 >> BITS(imm, 4, 0), 32));
  ...
  INSTPAT("0000000 ????? ????? 101 ????? 01100 11", srl    , R, R(rd) = src1 >> (src2 & SHAMT_MASK));
  INSTPAT("0000000 ????? ????? 000 ????? 01100 11", add    , R, R(rd) = src1 + src2); 
  INSTPAT("0000000 ????? ????? 001 ????? 01100 11", sll    , R, R(rd) = src1 << (src2 & SHAMT_MASK)); 
  ...
  INSTPAT("0000000 ????? ????? 101 ????? 01110 11", srlw   , R, R(rd) = SEXT((uint32_t)src1 >> BITS(src2, 4, 0), 32));
  INSTPAT("0100000 ????? ????? 101 ????? 01110 11", sraw   , R, R(rd) = SEXT((int32_t)src1 >> BITS(src2, 4, 0), 32));
  IFDEF(CONFIG_ISA64, INSTPAT("0000001 ????? ????? 000 ????? 01110 11", mulw   , R, R(rd) = SEXT((uint32_t)src1 * (uint32_t)src2, 32)));
  IFDEF(CONFIG_ISA64, INSTPAT("0000001 ????? ????? 100 ????? 01110 11", divw   , R, R(rd) = divw(src1, src2)));
  IFDEF(CONFIG_ISA64, INSTPAT("0000001 ????? ????? 101 ????? 01110 11", divuw  , R, R(rd) = divuw(src1, src2)));
  IFDEF(CONFIG_ISA64, INSTPAT("0000001 ????? ????? 110 ????? 01110 11", remw   , R, R(rd) = remw(src1, src2)));
  IFDEF(CONFIG_ISA64, INSTPAT("0000001 ????? ????? 111 ????? 01110 11", remuw  , R, R(rd) = remuw(src1, src2)));
  INSTPAT("0000001 ????? ????? 000 ????? 01100 11", mul    , R, R(rd) = (word_t)((unsigned __int128)(word_t)src1 * (unsigned __int128)(word_t)src2));
  ...
  INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh   , R, R(rd) = (word_t)(((__int128)(sword_t)src1 * (__int128)(sword_t)src2) >> 64););
  INSTPAT("0000001 ????? ????? 010 ????? 01100 11", mulhsu , R, R(rd) = (word_t)(((__int128)(sword_t)src1 * (__int128)(word_t)src2) >> 64););
  INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulhu  , R, R(rd) = (word_t)(((unsigned __int128)(word_t)src1 * (unsigned __int128)(word_t)src2) >> 64););
  INSTPAT("0000001 ????? ????? 100 ????? 01100 11", div    , R, if (src2 == 0) R(rd) = (word_t)-1;else if ((sword_t)src1 == (sword_t)0x8000000000000000 && (sword_t)src2 == -1) R(rd) = (word_t)0x8000000000000000;else R(rd) = (sword_t)src1 / (sword_t)src2;);
  INSTPAT("0000001 ????? ????? 101 ????? 01100 11", divu   , R, if (src2 == 0) R(rd) = (word_t)-1;else R(rd) = (word_t)((word_t)src1 / (word_t)src2););
  INSTPAT("0000001 ????? ????? 110 ????? 01100 11", rem    , R, if (src2 == 0) R(rd) = src1;else if ((sword_t)src1 == (sword_t)0x8000000000000000 && (sword_t)src2 == -1) R(rd) = 0;else R(rd) = (sword_t)src1 % (sword_t)src2;);
  INSTPAT("0000001 ????? ????? 111 ????? 01100 11", remu   , R, if (src2 == 0) R(rd) = src1;else R(rd) = (word_t)((word_t)src1 % (word_t)src2););

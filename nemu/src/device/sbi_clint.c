...
void update_sbi_time(uint64_t us){
    ...
    (*mtime_p) = us * 10; //RISCV CLIENT 10 MHz , get_time() is in us , so *10 to convert to 100 ns
    //! one hart
    uint64_t *mtimecmp_p = (uint64_t *)&sbi_clint_base[0x4000];
    if ((*mtime_p) >= (*mtimecmp_p)){
        cpu.mip |= (1 << 7); // Set the timer interrupt bit
    }else{
        cpu.mip &= (~(1 << 7)); // Clear the timer interrupt bit
    }
}
...
static void sbi_clint_io_handler(uint32_t offset, int len, bool is_write){
    ...
    if (offset == 0xbff8){ //mtime
        update_sbi_time(get_time());
        return;
    }
    if (offset >= 0x4000){ //mtimecmp
        offset -= 0x4000;
    ...
    if (is_write & (sbi_clint_base[offset] == 0x1)){ //set mtimecmp to 0, will cause a timer interrupt
        Log("sbi_clint: set mtimecmp to 0, will cause a timer interrupt\n");
        assert(0);

...
const uint32_t len2datamask_s[] = {0x0, 0xff, 0xffff, 0xffffff, 0xffffffff};
const uint32_t len2datamask_h[] = {0x0, 0xffffff00, 0xffff0000, 0xffffff00};

word_t cache_read(paddr_t addr, size_t len){
    // printf("the addr is " FMT_PADDR ", the len is %ld\n", addr, len);
    cache_access_num++;
    if (len == 8) {
        // RV64 8-byte aligned read: read two 32-bit words
        uint64_t lo = (uint64_t)__cache_read(addr);
        uint64_t hi = (uint64_t)__cache_read(addr + 4);
        return (hi << 32) | lo;
    }
    uint32_t data_s = (__cache_read(addr) >> addr_offset_bit_s(addr)) & len2datamask_s[len - (addr % len)];
    uint32_t data_h = 0;
    // assert((addr % len) == 0);
    if((addr%len)!=0){
        // printf("the addr is " FMT_PADDR ", the len is %ld\n", addr, len);
        // assert(0);
        data_h = (__cache_read(addr+4) << addr_offset_bit_h(addr)) & ((len==2)?0xff000000:len2datamask_h[addr%len]);
    }
    return (data_s | data_h);
}

void cache_write(paddr_t addr, size_t len, word_t data){
    cache_access_num++;
    if (len == 8) {
        // RV64 8-byte aligned write: split into two 32-bit writes
        assert((addr % 8) == 0);
        __cache_write(addr, (uint32_t)(data & 0xffffffff), 0xffffffff);
        __cache_write(addr + 4, (uint32_t)(data >> 32), 0xffffffff);
        return;
    }
    assert((addr % len) == 0);
    __cache_write(addr, data << addr_offset_bit_s(addr), len2datamask_s[len] << addr_offset_bit_s(addr));
}

...
extern word_t isa_reg_str2val(const char *name,bool *test);
	...
	if (p > q){
		...
		if(tokens[p].type==TK_NEG){
			...
			if(tokens[p+1].type==TK_NUM){
                word_t return_val;
                pmem_read(my_atoi(tokens[p + 1].str), &return_val);
                return return_val;
            }
				...
				if(point_reg_text==true){
                    word_t return_val;
                    pmem_read(point_reg_addr, & return_val);
                    return return_val;
                }
                else
					assert(0);
		...
		if(tokens[p].type==TK_NEG){
            ...
            word_t return_val;
            pmem_read(eval(p + 1, q), &return_val);
            return return_val;
        }
        else{
			assert(0);
